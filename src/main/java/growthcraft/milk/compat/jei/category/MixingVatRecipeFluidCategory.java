...
public class MixingVatRecipeFluidCategory implements IRecipeCategory<MixingVatFluidRecipe> {
    ...
    @Override
    public void draw(@NotNull MixingVatFluidRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull PoseStack stack, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, stack, mouseX, mouseY);
        Font font = Minecraft.getInstance().font;

        overlayHeated.draw(stack, 89, 48);

        font.drawWordWrap(FormattedText.of("Mixing Time " + TickUtils.toHoursMinutesSeconds(recipe.getProcessingTime())), 0, 63, 170, 0x404040);
    }
    
	@Override
	public Class<? extends MixingVatFluidRecipe> getRecipeClass() {
		return MixingVatFluidRecipe.class;
	}

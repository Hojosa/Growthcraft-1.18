...
public class BrewKettleRecipeCategory implements IRecipeCategory<BrewKettleRecipe> {
    ...
    public Component getTitle() {
        return new TranslatableComponent("block.growthcraft_cellar.brew_kettle");
    }
    ...
    @Override
    public void draw(BrewKettleRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, stack, mouseX, mouseY);

        try {
            if (recipe.isHeatRequired()) overlayHeated.draw(stack, 59, 44);
        } catch (Exception ex) {
            GrowthcraftCellar.LOGGER.error("Failure to draw heat texture for Brew Kettle recipe with JEI integration.");
        }

        Font font = Minecraft.getInstance().font;
        font.draw(stack, String.format("(%d%%)", recipe.getByProductChance()), 131, 26,0x404040);

        font.draw(stack, TickUtils.toHoursMinutesSeconds(recipe.getProcessingTime()), 59, 60,0x404040 );
    }
	...
	public ResourceLocation getUid() {
		return JEIGrowthcraftCellarModPlugin.BREW_KETTLE_RECIPE_TYPE.getUid();
	}

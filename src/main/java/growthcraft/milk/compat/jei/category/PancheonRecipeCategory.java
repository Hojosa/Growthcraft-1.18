...
public class PancheonRecipeCategory implements IRecipeCategory<PancheonRecipe> {
    ...
    public Component getTitle() {
        return new TranslatableComponent("jei.growthcraft_milk.category.pancheon");
    }
    ...
    @Override
    public void draw(PancheonRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, stack, mouseX, mouseY);
        Font font = Minecraft.getInstance().font;

        font.drawWordWrap(FormattedText.of("Processing Time " + TickUtils.toHoursMinutesSeconds(recipe.getRecipeProcessingTime())), 0, 63, 170, 0x404040);
    }
    
	@Override
	public Class<? extends PancheonRecipe> getRecipeClass() {
		return PancheonRecipe.class;
	}
